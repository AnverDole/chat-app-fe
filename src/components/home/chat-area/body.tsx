...
import ChatAreaFooter from "./footer";
import { useChatMaster, useChats } from "context/home-context";
import { VisibilitySensor } from "components/visibility-sensor";
import { Check, Check2, Check2All } from "react-bootstrap-icons";
...
interface Props {
    receiver: Friend;
    onSend: (messageId: string, receiverId: Key) => void;
    onSeen: (messageId: string, receiverId: Key) => void;
}



export default function ChatAreaBody({ receiver, onSend, onSeen }: Props) {
    const auth = useAuth();
    const chatMaster = useChatMaster();
    const lastMessageDate = useRef("");
    const determineDateBadgeVisibility = (message: Message): boolean => {
        const date = formatDate(new Date(message.created_at), "Y-m-d")

        if (date == lastMessageDate.current)
            return false;

        lastMessageDate.current = date
        return true;
    }
    ...
    const getDeliveryStatus = (message: Message): JSX.Element => {
        console.log(message.seen_at)
        if (message.seen_at) {
            return <Check2All className="ms-2 text-primary" />
        }
        if (message.downloaded_at) {
            return <Check2All className="ms-2 text-dark" />
        }

        return <Check2 className="ms-2 text-dark" />
    }
    ...
    const fetchMessages = async () => {
        ...
        try {
            ...
            const response = await getChat(receiver.id.toString(), auth.token);

            let chats = (response.chats ?? []).map(m => {
                m.created_at = !m.created_at ? null : new Date(m.created_at);
            ...
            chatMaster.context.updateAllChats({
                ...chatMaster.context.allChats,
                [receiver.id.toString()]: chats
            });
    ...
    return (
        <div
            ...
            <div className="position-relative">
                <div
                    ref={scrollRef}
                    className="pt-4 flex-1 overflow-y-auto space-y-2 mb-4 overflow-auto position-absolute top-0 bottom-0 start-0 end-0">
                        ...
                        <VisibilitySensor
                            ...
                            <div className="px-3">
                                {determineDateBadgeVisibility(msg) && <div
                                    ...
                                    className="d-flex justify-content-center fs-6 my-2">
                                    <p className="badge bg-success text-light fw-normal fs-6">{formatDate(msg.created_at, "Y-m-d")}</p>
                                </div>}
                                ...
                                {!msg.isMe && <div
                                    ...
                                    <div>
                                        <div className="ms-3 bg-light p-3 rounded-2">
                                            <p className="m-0 fw-bold">{receiver.first_name} {receiver.last_name}</p>
                                            <p className="m-0">{msg.message}</p>

                                        </div>
                                        <div className="d-flex justify-content-end mt-1">
                                            <p className="m-0 fw-light">{formatDate(msg.created_at, "h:i A")}</p>
                                        </div>
                                    </div>
                                </div>}
                                {msg.isMe && <div
                                    className="d-flex flex-row mb-3 justify-content-end">

                                    <div>
                                        <div className="ms-3 bg-light p-3 rounded-2">
                                            <p className="m-0 fw-bold">You</p>
                                            <p className="m-0">{msg.message}</p>

                                        </div>
                                        <div className="d-flex justify-content-end mt-1 align-items-center">
                                            <p className="m-0 fw-light">{formatDate(msg.created_at, "h:i A")}</p>
                    ...
                    {(chatMaster?.context.allChats?.[receiver.id.toString()] ?? [])?.length <= 0 && <div className="p-4 d-flex flex-column justify-content-center align-items-center h-100">
                        <svg
                            style={{
                                width: 200,
                                height: 200
                        ...
                        <p style={{ maxWidth: 350 }} className="text-center">Send a message to {receiver.first_name} {receiver.last_name} and start a conversation!</p>
            ...
            <ChatAreaFooter onSend={(message) => onSend(message, receiver?.id)} />
        </div>
    );
}
