...
import { Dash, PersonDash, PersonPlus, Plus, PlusCircle, Search } from 'react-bootstrap-icons';
import toast from 'react-hot-toast';
import { cancelRequest, findPeople, FriendStatus, FUser, sendRequest } from 'services/friendsService';
...
export default function FindFriendModal({
    ...
    const performSearch = async () => {
        ...
        try {
            const response = await findPeople(query, auth.token);
    ...
    const updateFriendStatus = (user: User, status: FriendStatus) => {
        setResults(
            results.map(r => r.id == user.id ? { ...user, friend_status: status, sent_by_me: auth.user.id == user.id } : r)
        );
    }
    ...
    return (
        <Modal show={show} onHide={onClose} centered>
            <Modal.Header closeButton>
                <Modal.Title>Find People</Modal.Title>
            </Modal.Header>
            <Modal.Body className='px-0'>
                ...
                {isSearchApplied && <>
                    {results.length > 0 && <div
                        className='px-3 d-flex flex-column overflow-y-auto'
                        style={{
                            height: "100%",
                            maxHeight: 450
                        }}>
                        {results.map(friend => <div
                            key={friend.id}
                            className={`p-2 mb-2 rounded-4 bg-light align-items-center border`}
                            style={{
                                display: "grid",
                                gridTemplateColumns: "60px auto max-content",
                            }}
                        >
                            <UserProfilePicture
                                user={friend}
                    ...
                    {results.length <= 0 && <div className="p-4 d-flex flex-column justify-content-center align-items-center h-100">
                        <svg
                            style={{
                                width: 200,
                                height: 200
                        ...
                        <p
                            ...
                            className="text-center">Hmm... couldn’t find any friends with that search. Try tweaking it a bit!</p>
                ...
                {!isSearchApplied && <>
                    {results.length <= 0 && <div className="p-4 d-flex flex-column justify-content-center align-items-center h-100">
                        ...
                        <p
                            ...
                            className="text-center">Use the search bar to find people by name or email and send them a friend request.</p>
            ...
            {
                isLoading && <div className="rounded position-absolute d-flex justify-content-center align-items-center bg-white" style={{ zIndex: 5, left: 0, right: 0, top: 0, bottom: 0 }}>
                    <LdsRollerLoader />
                </div>
            }
        </Modal >
