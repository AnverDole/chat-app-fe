...
export default function FriendList({ selectedFriend, onFriendSelected }: FriendListProps) {
    ...
    const fetchFriends = async () => {
        ...
        try {
            const response = await getAllFriends(query, auth.token);
    ...
    const unreadCountMapping = useMemo(() => {
        ...
        Object.keys(allChats)
            .forEach(userId => {
                const count = allChats?.[userId.toString()]
                    .filter(u => !u.seen_at)
                    .filter(u => !u.isMe)
                    .length
                map[userId.toString()] = count;
            });
        ...
        <div style={{
            ...
            <div className="mt-2 p-2 position-relative">
                <div className="position-absolute"
                    ...
                    {!isLoading && <>
                        {results.length > 0 && results.map(friend => <div
                            key={friend.id}
                            className={`p-2 btn btn-light mb-2 rounded-4 ${selectedFriend?.id == friend?.id ? 'bg-success text-light' : ''} `}
                            style={{
                                display: "grid",
                            ...
                            <div className="text-start">
                                <div className="d-flex flex-row align-items-center">
                                    ...
                                    {unreadCountMapping?.[friend.id.toString()] > 0 && <span className="badge bg-dark rounded-pill ms-2">{unreadCountMapping?.[friend.id.toString()]}</span>}
                                </div>
